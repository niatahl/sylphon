...
public class MissionDefinition implements MissionDefinitionPlugin {
	...
	public void defineMission(MissionDefinitionAPI api) {
		...
		api.initFleet(FleetSide.PLAYER, "SRD", FleetGoal.ATTACK, false);
...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "Sylphon RnD fleet portfolio");
		...
		for (String testID : Global.getSettings().getAllVariantIds()) {
			if (testID.contains("SRD_") && Global.getSettings().getVariant(testID).getSource() == VariantSource.HULL && !Global.getSettings().getVariant(testID).getHullSize().equals(ShipAPI.HullSize.FIGHTER)) {
