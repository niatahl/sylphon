...
public class SRD_CreeperDevice extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        for (int i = 0; i < (500 * Global.getCombatEngine().getElapsedInLastFrame()); i++) {
            SRD_FakeSmokePlugin.addFakeSmoke(MathUtils.getRandomNumberInRange(0.38f, 0.67f), MathUtils.getRandomNumberInRange(70f, 90f),
                    MathUtils.getRandomPointInCircle(shadowPos, SHADOW_DISTANCE_DIFFERENCE), MathUtils.getRandomPointInCircle(null, 10f),
                    MathUtils.getRandomNumberInRange(-12f, 12f), 0.7f, new Color(0f, 0f, 0f));
        }
    }
    ...
    public void unapply(MutableShipStatsAPI stats, String id) {
        ...
        if (ship.getSystem().getEffectLevel() <= 0f) {
            ...
            for (int i = 0; i < 100; i++) {
                SRD_FakeSmokePlugin.addFakeSmoke(MathUtils.getRandomNumberInRange(0.1f, 0.78f), MathUtils.getRandomNumberInRange(50f, 90f),
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        if (index == 0) {
            return new StatusData("lurking...", false);
        }
        return null;
    }
}