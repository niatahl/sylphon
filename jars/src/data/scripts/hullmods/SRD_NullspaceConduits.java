...
public class SRD_NullspaceConduits extends BaseHullMod {
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
        stats.getEnergyWeaponFluxCostMod().modifyMult("SRD_NullspaceConduitsID", ENERGY_FLUX_COST_PERCENT_DIFF);
        stats.getBeamWeaponFluxCostMult().modifyMult("SRD_NullspaceConduitsID", BEAM_FLUX_COST_PERCENT_DIFF / ENERGY_FLUX_COST_PERCENT_DIFF);
    ...
    public String getDescriptionParam(int index, HullSize hullSize, ShipAPI ship) {
        if (index == 0 && ship.getHullSpec().getHullId().contains("SRD_")) {
            return "";
        } else if (index == 0) {
            return "";  //This was once an easter egg, but no more. Left for compatibility, and making it easier to put the easter egg back in
        }
        if (index == 1 && ship.getHullSpec().getHullId().contains("SRD_")) {
            return "";
        } else if (index == 1) {
            return "";  //This was once an easter egg, but no more. Left for compatibility, and making it easier to put the easter egg back in
        }
        if (index == 2) return "" + (int)(MAX_SPEED_BONUS_PERCENTAGE) + "%";
        if (index == 3) return "" + (int)Math.ceil(100f-(ENERGY_FLUX_COST_PERCENT_DIFF * 100f)) + "%";
        if (index == 4) return "" + (int)((1f-BEAM_FLUX_COST_PERCENT_DIFF)*100f) + "%";
        if (index == 5) return "while overloading, nullspace energy discharges from the conduits will harm the ship and any nearby objects";
        if (index == 6) return "" + (int)Math.ceil(NULLSPACE_DAMAGE_REDUCTION*100f) + "%";
        if (index == 7) return "Advanced Optics";
        return null;
    }
}